...
st.set_page_config(
    page_title="Knot Codz - Img Generation tool",
    page_icon="KnotCodz-Logo1.png",
    layout="wide",
    initial_sidebar_state="collapsed"
)

# Custom CSS for Knot Codz theme - Black & Green
st.markdown("""
<style>
    /* Main App Background */
    .stApp {
    ...
    p, .stMarkdown, .stText, .stCaption, label, span, div, li {
...
api_key = st.secrets["BRIA_API_KEY"]
...
def main():
    st.markdown(
            ...
            <div style='display: flex; align-items: center; gap: 1rem; margin-bottom: 2rem;'>
                <img src='https://www.knotcodz.com/KnotCodz-Logo.png' alt='Knot Codz Logo' style='height: 48px; width: 48px; border-radius: 8px;'/>
                <h1 style='margin: 0; font-size: 2.5rem; color: #22c55e; font-weight: 800; letter-spacing: -0.02em;'>Knot Codz - Img Generation tool</h1>
            </div>
            """,
            unsafe_allow_html=True
        )
    ...
    with tabs[0]:
        ...
        # Use a single column for prompt and button to reduce space
        col = st.container()
        with col:
            ...
            with col1:
                ...
                if edit_option == "Create Packshot":
                    ...
                    with col_b:
                        ...
                        shadow_blur = st.slider("Shadow Blur", 0, 50, 15 if shadow_type and shadow_type.lower() == "regular" else 20)
                        ...
                        with st.spinner("Adding shadow effect..."):
                            try:
                                result = add_shadow(
                                    api_key=st.session_state.api_key,
                                    image_data=uploaded_file.getvalue(),
                                    shadow_type=shadow_type.lower() if shadow_type else "natural",
                            ...
                            with st.spinner("Generating lifestyle shot..."):
                                try:
                                    ...
                                    result = lifestyle_shot_by_text(
                                        ...
                                        placement_type=(placement_type.lower().replace(" ", "_") if placement_type else "original"),
                                        num_results=num_results,
                                        sync=sync_mode,
                                        fast=fast_mode,
                                        optimize_description=optimize_desc,
                                        shot_size=[int(shot_width), int(shot_height)] if placement_type != "Original" else [1000, 1000],
                                        original_quality=original_quality,
                                        exclude_elements=exclude_elements if not fast_mode else None,
                                        manual_placement_selection=manual_placements,
                                        padding_values=[int(pad_left), int(pad_right), int(pad_top), int(pad_bottom)] if placement_type == "Manual Padding" else [0, 0, 0, 0],
                                        foreground_image_size=[int(fg_width), int(fg_height)] if placement_type == "Custom Coordinates" else None,
                                        foreground_image_location=[int(fg_x), int(fg_y)] if placement_type == "Custom Coordinates" else None,
                                        force_rmbg=force_rmbg,
                                        content_moderation=content_moderation,
    ...
    # Add footer for Knot Codz branding
    st.markdown("""
    <div style='text-align: center; margin-top: 3rem; padding: 1.5rem 0; border-top: 1px solid #1e293b;'>
        <span style='color: #e5e7eb; font-size: 1.1rem;'>Developed by <a href='https://www.knotcodz.com/' style='color: #22c55e; text-decoration: none; font-weight: bold;' target='_blank'>Knot Codz 💚</a></span>
    </div>
    """, unsafe_allow_html=True)
